  ...
  return (
    <nav className="sticky top-0 inset-x-0 z-30 border-b border-slate-800/60 bg-slate-950/85 backdrop-blur-xl">
      <div className="max-w-6xl mx-auto px-3 sm:px-4 lg:px-6">
        <div className="flex items-center justify-between gap-3 sm:gap-4 py-3 sm:py-3 md:py-4">
          <span className="text-sm sm:text-base md:text-lg font-semibold tracking-wide text-slate-100">
            Ahmad Almassawa
          </span>
          ...
          <button
            className="md:hidden inline-flex items-center justify-center rounded-full p-2 text-slate-200 ring-1 ring-slate-700/60 transition-smooth hover:bg-slate-800/80 hover:text-blue-200"
      ...
      >
        <div className="max-w-6xl mx-auto px-3 sm:px-4 pb-3 pt-2 flex flex-col gap-1.5">
          {items.map((key) => {
            const isActive = active === key;
            return (
              <button
                key={key}
                className={`w-full rounded-xl px-4 py-2.5 text-left text-sm font-medium transition-smooth ${
                  isActive
